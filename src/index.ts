...
const logToFile = (message: string) => {
  ...
  if (fs.existsSync(logFile) && !(global as any).hasOwnProperty('logCleared')) {
    fs.unlinkSync(logFile);
    (global as any)['logCleared'] = true;
  }
...
async function getGitDiff(repoUrl: string, base: string, head: string) {
...
async function getGitDiffUriFromLogs(projectId: string, deploymentName: string) {
